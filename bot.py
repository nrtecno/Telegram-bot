...
def add_monthly_user(user_id):
    ...
    if user_id not in stats[current_month]:
        stats[current_month].append(user_id)
...
def get_monthly_users_count():
    ...
    with open(STATS_FILE, "r") as f:
        stats = json.load(f)
    return len(stats.get(current_month, []))
        ...
        if 'text' in msg:
            ...
            if text == '/start':
                if not is_subscribed(uid):
                    ...
                    add_monthly_user(uid)
                    set_user_state(uid, "waiting_url")
                    
                    # Menu with /start at bottom
                    monthly_count = get_monthly_users_count()
                    menu_markup = {
                        "inline_keyboard": [
                            [{"text": "📊 Monthly Users: " + str(monthly_count), "callback_data": "stats"}],
                            [{"text": "🚀 /start", "callback_data": "start"}]
                        ]
                    }
                    send_message(uid, f"✅ <b>Welcome to SecureShare!</b>\n\n📌 Send me any URL to get started.", reply_markup=menu_markup)
            elif text == '/help':
...
👑 Developed by: @nrtecno2"""
            ...
            try:
                ...
                # Send link to storage channel as well
                if CHANNEL_ID:
                    send_message(CHANNEL_ID, f"🔗 New link generated by user {uid}:\n{link}")
